...
- **`spl_token_2022`** — Token-2022 TLV / 固定长度扩展解析（v0.1，
  仅解析，无 ix builder / CPI）
...
- **`solana_codec`** — shortvec、Borsh 基础类型 / string / bytes、
  bincode `COption` 等显式字节布局 helper。不做反射式通用序列化。
...
- **`solana_loader_v3`** — 当前 v0.1 为 Upgradeable BPF Loader v3
  ...
  instruction builders，并用 `solana-loader-v3-interface = 6.1.1`
  固定 Rust parity。
...
- **`solana_vote`** — 当前 v0.1 为 Vote Program initializeAccount、
  ...
  withdraw instruction builders；runtime vote submission、vote-state update
  和 tower sync 后续补。
...
- **`solana_client`** — 当前 v0.1 为 JSON-RPC request / response codec、
  ...
  subscription request / notification surface 和 typed RPC error
  normalization；
  concrete WebSocket socket adapter 和 transport-level deadline enforcement
  后续补。
...
- **`solana_tx`** — 当前 v0.1 为 legacy message 编译、legacy/v0 message
  serialization 和 signed transaction byte serialization；保持为底层 message
  codec，不负责 RPC 或 wallet。
- **`solana_transaction_builder`** — 当前 v0.1 为 legacy transaction
  ...
  prelude 和 Compute Budget + System transfer 组合 helper。
- **`solana_wallet`** — 当前 v0.1 为 Solana CLI keypair JSON
  parse/write、BIP39 PBKDF2 seed derivation、caller-supplied wordlist
  checksum validation、Solana hardened derivation path parse/write、
  SLIP-0010 Ed25519 child derivation、wallet adapter boundary 和
  encrypted keystore envelope parse/write / AEAD payload helper；bundled
  BIP39 wordlist 后续补。
...
2. 包名以 `spl_` 开头 → **通常双用**，host 和 BPF 都能编；但像
   `spl_token_2022` 这样的 parsing-only 包可以只暴露共享的只读解析
   API，不提供 `instruction.zig` / `cpi.zig`；interface-only SPL 包
...
│   ├── tlv.zig                  ← parsing-only 包的 TLV 扫描
│   └── extension.zig            ← parsing-only 包的扩展视图
└── examples/                    ← 该包的演示程序
```
