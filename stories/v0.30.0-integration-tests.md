...
- [ ] Signature Base58 编解码与 Rust 完全一致 (需要更新 format 方法签名)
- [ ] InstructionError bincode 序列化与 Rust 完全一致 (Rust 版本兼容性问题)
- [ ] TransactionError bincode 序列化与 Rust 完全一致 (Rust 版本兼容性问题)
- [ ] UpgradeableLoaderInstruction bincode 序列化与 Rust 完全一致
- [ ] PDA 派生结果与 Rust 完全一致 (需要运行时 seeds 支持)
- [ ] EpochInfo JSON 格式与 RPC 响应一致

### 测试覆盖

- [ ] 每种 InstructionError 变体 (约 50 个) - 已生成测试向量
- [ ] 每种 TransactionError 变体 (约 35 个) - 已生成测试向量
- [ ] 每种 UpgradeableLoaderInstruction 变体 (10 个) - 已生成测试向量
- [x] 边界条件和特殊值测试 (PublicKey/Hash)
...
4. ✅ PublicKey Base58 编解码测试
5. ✅ Hash Base58 编解码测试
6. ✅ 更新 test.sh 支持集成测试

### 待完成工作

1. Signature Base58 测试 (需要更新 SDK 中 Signature.format 签名)
2. PDA 派生测试 (需要运行时 seeds 支持)
3. bincode 序列化测试 (Rust 版本兼容性问题待解决)
