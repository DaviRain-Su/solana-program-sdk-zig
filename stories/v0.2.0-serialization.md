...
> **Note**: Marked as out of scope - client-only utilities, not needed for on-chain program development.

- [x] ~~Common serialization helpers~~ (Out of scope - client-only)
- [x] ~~Buffer management utilities~~ (Out of scope - client-only)
- [x] ~~Integration with short_vec~~ (Out of scope - client-only)
