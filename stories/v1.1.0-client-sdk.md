...
### Phase 1: 基础设施 ✅
- [x] 创建 `client/` 目录结构
- [x] 实现 `error.zig` - ClientError, RpcError
- [x] 实现 `types.zig` - Response<T>, RpcResponseContext
- [x] 实现 `commitment.zig` - CommitmentConfig
- [x] 实现 `json_rpc.zig` - HTTP JSON-RPC 客户端
- [x] 基础测试通过
...
### Phase 5: 便捷方法 ✅
- [x] `_with_commitment` 变体 (已在 P0-P2 中实现)
- [x] `sendAndConfirmTransaction` - 发送并等待确认
- [x] `confirmTransaction` - 等待交易确认
- [x] `pollForSignatureStatus` - 轮询签名状态
- [x] `getNewBlockhash` - 获取新的区块哈希
- [x] `isHealthy` - 检查节点健康状态
- [x] `getBalanceInSol` - 获取 SOL 余额（非 lamports）
