...
| `spl_token_2022` | released v0.1 interface-core | Token-2022 fixed/TLV extension views, base mint/account/authority + reallocate + withdrawExcessLamports + Transfer Fee / MintCloseAuthority / DefaultAccountState / MemoTransfer / NonTransferable / CpiGuard / InterestBearingMint / PermanentDelegate / Pausable / pointer / TransferHook / ScaledUiAmount instruction builders, and Rust parity fixtures |
...
| `solana_codec` | v0.1 codec primitives | shortvec, Borsh primitives/string/bytes, bincode string/options, bincode `COption` |
...
| `solana_transaction_builder` | v0.1 transaction assembly | compile/sign/serialize legacy/v0 transactions, supplied-ALT selection, durable nonce pairs, compute-budget System/SPL Token transfer preludes, and ATA+token transfer helpers |
...
1. Broader transaction-level ergonomics: richer app-specific instruction
   bundles beyond nonce, compute-budget System/SPL Token transfer, and
   ATA+token transfer flows.
2. Codec adoption: migrate remaining package-local Borsh string, bincode
   `COption`, and shortvec helpers onto `solana_codec` where that does not
   disturb verified parity fixtures.
3. Wider program ecosystem: confidential-transfer proof-generation
   ergonomics and additional SPL extension surfaces.
...
5. Crypto/syscall parity: secp256r1, BLS/MCL-style helpers, and any Solana
