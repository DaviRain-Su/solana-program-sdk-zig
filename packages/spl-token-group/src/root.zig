//! `spl_token_group` — on-chain/interface scaffold for the SPL
//! Token Group interface package.
//!
...
test "@import(\"spl_token_group\") exposes only interface scaffold declarations" {
...
test "source-review guards keep spl_token_group interface scoped" {
    const root_source = @embedFile("root.zig");
    try expectContains(root_source, "spl_token_group");
