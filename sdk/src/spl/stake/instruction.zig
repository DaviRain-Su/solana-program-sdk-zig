//! Zig implementation of Solana Stake program instruction types and builders
//!
//! Rust source: https://github.com/solana-program/stake/blob/master/interface/src/instruction.rs
//!
//! This module provides:
//! - Instruction TYPE DEFINITIONS (StakeInstruction enum)
...
// Re-export sysvar IDs for convenience
pub const CLOCK_SYSVAR = sysvar_id.CLOCK;
pub const RENT_SYSVAR = sysvar_id.RENT;
pub const STAKE_HISTORY_SYSVAR = sysvar_id.STAKE_HISTORY;
...
/// Maximum size of instruction data for Initialize instruction
...
test "initialize: creates correct instruction data" {
    ...
    const withdrawer_key = PublicKey.from([_]u8{3} ** 32);
...
test "initializeChecked: creates correct instruction" {
    ...
    const result = initializeChecked(stake_account, staker, withdrawer_key);
