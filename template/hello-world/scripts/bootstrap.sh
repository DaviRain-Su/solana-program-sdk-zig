...
if SOLANA_ZIG_BIN="$($ROOT_DIR/scripts/ensure-solana-zig.sh 2>/dev/null)"; then
  echo "Found project-compatible solana-zig fork: $SOLANA_ZIG_BIN"
  echo "Recommended: export SOLANA_ZIG_BIN=\"$SOLANA_ZIG_BIN\""
else
  SOLANA_ZIG_BIN=""
  echo "No project-compatible solana-zig fork found."
...
echo ""
echo "Next steps:"
echo "  1. Edit src/main.zig"
echo "  2. Run host tests: zig build test --summary all"
echo "  3. Run program-test with a verified solana-zig fork"
