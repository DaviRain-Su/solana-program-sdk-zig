...
- [x] ~~Secp256k1 完整签名验证~~ (Deferred - instruction offset serialization tested, full verification requires external library)
...
1. ⏸️ InstructionError bincode 序列化 - 需要确保 Rust/Zig enum 编号完全一致，复杂度高 (现有测试已覆盖主要用例)
2. ⏸️ TransactionError bincode 序列化 - 同上 (13 vectors 已验证常用变体)
