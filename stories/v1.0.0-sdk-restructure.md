...
- [ ] 创建 `sdk/` 目录结构
- [ ] 移动共享类型模块到 `sdk/src/`
- [ ] 修改 `public_key.zig` 移除 syscall 依赖
- [ ] 修改 `instruction.zig` 移除 CPI 代码
- [ ] 创建 `sdk/build.zig` 和 `sdk/build.zig.zon`
- [ ] 创建 `sdk/src/root.zig` 导出所有类型
- [ ] `./solana-zig/zig build test` 在 sdk/ 目录通过
- [ ] 所有 SDK 模块无 syscall 依赖

### Phase 2: 重构 solana-program-sdk-zig

- [ ] 修改 `build.zig` 依赖 `sdk/`
- [ ] 更新所有模块的导入路径
- [ ] 保留 syscall 相关功能在 program 模块
- [ ] 为 `public_key` 提供 syscall 版本的 PDA 函数
- [ ] `./solana-zig/zig build test` 通过 (375/375)
- [ ] 功能行为与重构前一致
