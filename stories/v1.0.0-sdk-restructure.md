...
| 模块 | 说明 | 状态 |
|------|------|------|
| `rpc/client.zig` | JSON-RPC Client | ✅ 已完成 (v1.1.0) |
| `rpc/types.zig` | RPC 类型定义 | ✅ 已完成 (v1.1.0) |
| `connection.zig` | Connection API | ✅ 已完成 (v1.1.0) |
| `transaction/builder.zig` | 交易构建器 | ✅ 已完成 (v1.1.0) |
| `transaction/signer.zig` | 交易签名 | ✅ 已完成 (v1.1.0) |
| `wallet/keypair.zig` | 完整 Keypair (含签名) | ✅ 已完成 (sdk/) |
...
- [x] 删除 `src/` 中的纯模块 (hash, signature, keypair, bincode, borsh, short_vec, error, native_token, nonce)
- [x] 重写 `src/public_key.zig` 重新导出 SDK 类型 + syscall 加速的 PDA 函数
- [x] 更新所有 `src/` 文件使用 `@import("solana_sdk")` 导入 SDK 类型
- [x] `./solana-zig/zig build test` 通过 (286/286 tests)
- [x] `sdk/` 测试独立通过 (105/105 tests)
- [x] 总测试覆盖: 391 tests (286 + 105)
- [x] 功能行为与重构前一致
- [x] 导出 `solana_sdk` 模块供消费者选择使用

### Phase 3: 创建 solana-client-sdk-zig ✅

- [x] 创建 `client/` 目录结构
- [x] 实现 RPC Client (52 个方法)
- [x] 实现 Connection API
- [x] 实现交易构建器
- [x] 添加测试 (108 tests)
...
- [x] 提交: "refactor: extract shared SDK types" (ab2ca9b)

### Step 3: Phase 2 实施 ✅
- [x] 修改 build.zig 添加 SDK 依赖
- [x] 修改 build.zig.zon 添加本地路径依赖
- [x] 导出 solana_sdk 模块
- [x] 提交: "refactor: program-sdk depends on SDK" (f2b7ffe)
- [x] 删除 src/ 中重复的纯模块
- [x] 更新 src/ 文件从 SDK 导入类型
- [x] 测试完整功能 (SDK: 105, Program: 286, Total: 391)
- [x] 提交: "refactor: complete SDK extraction with proper module imports"

### Step 4: Phase 3 实施 ✅
- [x] 创建 client 目录
- [x] 实现 RPC client (52 个方法)
- [x] 实现 transaction builder 和 signer
- [x] 测试 (108 tests)
- [x] 提交: "feat: add client SDK" (参见 v1.1.0)
