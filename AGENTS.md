...
- 为问题构造 1–3 个合理的假设，并按可能性排序
  - 先验证最可能的假设
  - 不要过早排除低概率但高风险的可能性
- 在实现或分析过程中，如果新的信息否定原有假设，需要更新假设集合并相应调整方案
...
在多数情况下，应优先尝试基于现有信息做出合理假设并推进，而不是因为细枝末节停滞不前。

### 精确性与落地性

- 保持推理与建议高度贴合当前具体情境，而不是泛泛而谈
- 当依据某条约束/规则做决策时，可以用简洁自然语言说明「依据了哪些关键约束」，但不必重复整个提示词的原文
...
| 复杂度 | 特征 | 策略 |
|--------|------|------|
| **trivial** | 简单语法问题、单个 API 用法、小于约 10 行的局部修改 | 直接回答，不必显式进入 Plan/Code 模式 |
| **moderate** | 单文件内的非平凡逻辑、局部重构、简单性能/资源问题 | 使用 Plan/Code 工作流 |
| **complex** | 跨模块或跨服务的设计问题、并发与一致性、复杂调试 | 必须使用 Plan/Code 工作流 |
...
1. 自上而下分析问题，找出根因和核心路径，而不是只对症状打补丁
...
5. 避免给出本质相同的 Plan：如果新方案与上一版只有细节差异，只说明差异与新增内容即可
...
1. 本回复的主要内容必须是具体实现（代码、补丁、配置等），而不是继续长篇讨论计划
2. 在给出代码前，简要说明：
   - 将修改哪些文件/模块/函数
   - 每个修改的大致目的
3. 偏好**最小、可审阅的修改**
...
**模式切换规则**：
- 当用户在自然语言中使用 "实现"、"落地"、"按方案执行"、"开始写代码"、"帮我把方案 A 写出来" 等表述时：
  - 必须视为用户在明确请求进入 **Code 模式**
  - 在该回复中立即切换到 Code 模式并开始实现
  - 禁止再次提出同一选择题或再次询问用户是否同意该方案
...
- 严格遵循各语言社区的惯用写法与最佳实践（Zig、Rust、Roc 等）
- 主动留意并指出以下"坏味道"：
  - 重复逻辑/复制粘贴代码
  - 模块间耦合过紧或循环依赖
  - 改动一处导致大量无关部分破坏的脆弱设计
...
- 在给出较大代码片段时，默认该代码已经过对应语言的自动格式化工具处理
- 注释：
  - 仅在行为或意图不明显时添加注释
...
对于每个用户问题（尤其是 non-trivial 任务），回答应包含：

1. **直接结论**：用简洁语言先回答"应该怎么做/当前最合理的结论是什么"
...
3. **可选方案或视角**：若存在明显替代实现，简要列出 1–2 个选项
...
- **禁止**主动建议使用重写历史的命令（`git rebase`、`git reset --hard`、`git push --force`），除非用户明确提出
- 对明显具有破坏性的操作必须在命令前明确说明风险
- 优先使用 `gh` CLI 进行 GitHub 交互
- 如有可能，给出更安全的替代方案（如先备份、先 `git status`、使用交互式命令等）
...
**🚨 强制约束**: **绝对禁止在有测试失败的情况下提交代码**。所有测试必须 100% 通过才能进行 git commit 操作。
...
**核心原则**: 重构先行，测试验证，新功能后加。绝对禁止"一口吃成胖子"。

#### 重构优先原则

当进行架构调整或代码重组时，必须遵循以下顺序：
...
- ❌ 跳过测试验证直接进入下一阶段
- ❌ 在测试失败的状态下继续开发
- ❌ 将重构和新功能混在同一个 commit 中

**✅ 必须遵循**:
- ✅ 每个阶段完成后必须运行 `zig build test` 验证
- ✅ 重构完成后单独提交，再开始新功能开发
- ✅ 如果重构导致测试失败，必须先修复再继续
- ✅ 新功能开发前确认基础代码稳定可用
...
**场景：重组仓库结构并添加 Client 功能**

```
正确做法：
1. 移动 on-chain 模块到 program/ 目录
2. 更新所有导入路径
3. 运行测试，确保通过
4. 提交: "refactor: reorganize project structure"
5. 然后开始实现 client 功能
6. 运行测试，确保通过
7. 提交: "feat: add RPC client implementation"

错误做法：
1. 移动文件的同时编写新的 RPC client 代码
2. 到处都是编译错误
3. 不确定哪些错误是重构导致，哪些是新代码导致
4. 调试困难，浪费时间
```
...
> 本节规范基于 [Xuanwo's AGENTS.md](https://gist.github.com/Xuanwo/fa5162ed3548ae4f962dcc8b8e256bed)，适用于项目中的 Rust 代码开发。
...
version = "0.1.0"
edition = "2021"
rust-version = "1.75"  # 明确指定最低 Rust 版本
...
#### 代码质量
- [ ] `cargo fmt` 格式化通过
- [ ] `cargo clippy` 无警告（或已合理处理）
- [ ] `cargo test` 所有测试通过
- [ ] `cargo doc` 文档生成无警告
- [ ] 公共 API 有完整文档注释

#### 安全性
- [ ] 无 `unwrap()`/`expect()` 在非测试代码中（除非有充分理由）
- [ ] 无 `unsafe` 代码（除非有充分理由并有详细注释）
- [ ] 依赖已通过 `cargo audit` 检查
